...
client.on('messageCreate', async message => {
	...
	if (message.content.startsWith('/zvz ')) {
		console.log(message)
		let newMsg = message.content.replace('/zvz', '').replace(' ', '');
		...
		if (isNaN(newMsg) && eventId.length == 1) {
			message.reply("Escribe correctamente la ID o LINK de la batalla - /zvz <link> o <id>")
			return;
		}
		if (!isNaN(newMsg)) {
		...
		getData("api.aotools.net", '/v2/battles/' + `${eventIdParsed}`, async function (data) {
			...
			if (IsJsonString(data)) {
				...
				return message.reply("Batalla no encontrada");
			}
			...
			context.fillText(`Alianzas: ${arrayAlliances.length} ┊ Gremios: ${arrayGuilds.length} ┊ Jugadores: ${Object.keys(allPlayers).length} ┊ Asesinatos: ${totalKills} ┊ Fama total: ${totalFame}`, canvas.width/2, 92);
			...
			const embed = new EmbedBuilder()
				.setAuthor({
					name: "HYDRA BATTLE REPORT", 
					iconURL: "https://i.imgur.com/IstDNHs.png"
                })
			...
			try {
				message.channel.send({ embeds: [embed], content: '‎ ', files: [attachment] }).catch(console.error);
			} catch {
				console.log("error2")
			}
	...
	// This part below work with discordjs v12
		...
		let newMsg = message.content.replace('/zvzs ', '');
		...
		context.fillStyle = '#ffffff';
		context.textAlign = "center";
		context.fillText(`Alianzas/Gremios: ${arrayAlliances.length}/${arrayGuilds.length} | Jugadores: ${arrayPlayersG.length} | Asesinatos: ${totalKills} | Killfame: ${totalFame}`, canvas.width / 2, 92);
